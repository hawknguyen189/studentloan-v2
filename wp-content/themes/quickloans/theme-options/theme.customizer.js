/* global quickloans_color_schemes, quickloans_dependencies, Color */
...
( function( api ) {
	...
	for (var i in quickloans_color_schemes) {
		cssTemplate[i] = wp.template( 'quickloans-color-scheme-'+i );
	}
	// Add Template with theme fonts
	cssTemplate['theme_fonts'] = wp.template( 'quickloans-fonts' );
	
	// Set initial state of controls
	api.bind('ready', function() {

		// Add 'reset' button
		jQuery('#customize-header-actions #save').before('<input type="button" class="button customize-action-reset" value="'+quickloans_customizer_vars['msg_reset']+'">');
		jQuery('#customize-header-actions .customize-action-reset').on('click', function(e) {
			if (confirm(quickloans_customizer_vars['msg_reset_confirm'])) {
		...
		jQuery('#customize-theme-controls [id^="customize-control-load_fonts"]').on('focusout', quickloans_customizer_update_load_fonts);		

		// Click on the actions button
		jQuery('#customize-theme-controls .control-section .customize-control-button input[type="button"]').on('click', quickloans_customizer_actions);

		// Check dependencies in the each section
		jQuery('#customize-theme-controls .control-section').each(function () {
			quickloans_customizer_check_dependencies(jQuery(this));
		});
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		quickloans_customizer_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		quickloans_customizer_refresh_preview(obj);
	});

	// Check for dependencies
	function quickloans_customizer_check_dependencies(cont) {
		cont.find('.customize-control').each(function() {
			var id = jQuery(this).attr('id');
			if (id == undefined) return;
			id = id.replace('customize-control-', '');
			var depend = false;
			for (var fld in quickloans_dependencies) {
				if (fld == id) {
					depend = quickloans_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('quickloans_options_no_use');
				} else {
					jQuery(this).hide().addClass('quickloans_options_no_use');
				}
			}
		});
	}
	...
	function quickloans_customizer_refresh_preview(obj) {
		...
		if (updateCSS) {
			...
			// Any theme fonts parameter is changed
				...
				for (opt in quickloans_theme_fonts) {
					for (rule in quickloans_theme_fonts[opt]) {
						if (opt+'_'+rule == id) {
							// Store new value in the color table
							quickloans_customizer_update_theme_fonts(opt, rule, val);
							processed = true;
							break;
						}
					}
			...
			// Refresh CSS
			if (processed) quickloans_customizer_update_css();
		}
	...
	// Actions buttons
	function quickloans_customizer_actions(e) {
	...
	function quickloans_customizer_update_theme_fonts(opt, rule, value) {
		quickloans_theme_fonts[opt][rule] = value;
	}
	
	// Change theme fonts options if load fonts is changed
	function quickloans_customizer_update_load_fonts() {
		var opt_list=[], i, tag, sel, opt, name='', family='', val='', new_val = '', sel_idx = 0;
		updateCSS = false;
		for (i=1; i<=quickloans_customizer_vars['max_load_fonts']; i++) {
			name = api('load_fonts-'+i+'-name')();
			if (name == '') continue;
			family = api('load_fonts-'+i+'-family')();
			opt_list.push([name, family]);
		}
		for (tag in quickloans_theme_fonts) {
	...
	function quickloans_customizer_update_css() {
		...
		var fonts = quickloans_customizer_add_theme_fonts(quickloans_theme_fonts);

		// Make styles and add into css
		css += quickloans_customizer_prepare_html_value(cssTemplate['theme_fonts']( fonts ));

		// Add colors
		for (var scheme in quickloans_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in quickloans_color_schemes[scheme].colors) {
				colors[i] = quickloans_color_schemes[scheme].colors[i];
			}
			
			// Make theme specific colors and tints
			colors = quickloans_customizer_add_theme_colors(colors);
	...
	function quickloans_customizer_add_theme_fonts(fonts) {
		...
		for (var tag in fonts) {
			...
			rez[tag+'_font-size'] = typeof fonts[tag]['font-size'] != 'undefined' && fonts[tag]['font-size'] != 'inherit'
												? 'font-size:' + quickloans_customizer_prepare_css_value(fonts[tag]['font-size']) + ";"
												: '';
			...
			rez[tag+'_margin-top'] = typeof fonts[tag]['margin-top'] != 'undefined' && fonts[tag]['margin-top'] != '' && fonts[tag]['margin-top'] != 'inherit'
												? 'margin-top:' + quickloans_customizer_prepare_css_value(fonts[tag]['margin-top']) + ";"
												: '';
			rez[tag+'_margin-bottom'] = typeof fonts[tag]['margin-bottom'] != 'undefined' && fonts[tag]['margin-bottom'] != '' && fonts[tag]['margin-bottom'] != 'inherit'
												? 'margin-bottom:' + quickloans_customizer_prepare_css_value(fonts[tag]['margin-bottom']) + ";"
												: '';
		}
		return rez;
	}
	...
	// Add ed to css value
	function quickloans_customizer_prepare_css_value(val) {
		if (val != '' && val != 'inherit') {
			var ed = val.substr(-1);
			if ('0'<=ed && ed<='9') val += 'px';
		}
	...
	function quickloans_customizer_prepare_html_value(val) {
