...
------------------------------------------------------------------------------- */
if (!function_exists('quickloans_trx_addons_theme_specific_setup1')) {
	add_filter( 'trx_addons_filter_components_editor', 'quickloans_trx_addons_theme_specific_components');
	function quickloans_trx_addons_theme_specific_components($enable=false) {
		return false;
	}
}

if (!function_exists('quickloans_trx_addons_theme_specific_setup1')) {
	add_action( 'after_setup_theme', 'quickloans_trx_addons_theme_specific_setup1', 1 );
	add_action( 'trx_addons_action_save_options', 'quickloans_trx_addons_theme_specific_setup1', 1 );
	function quickloans_trx_addons_theme_specific_setup1() {
		if (quickloans_exists_trx_addons()) {
			add_filter( 'trx_addons_cv_enable',					'quickloans_trx_addons_cv_enable');
			add_filter( 'trx_addons_demo_enable',				'quickloans_trx_addons_demo_enable');
			add_filter( 'trx_addons_filter_edd_themes_market',	'quickloans_trx_addons_edd_themes_market_enable');
			add_filter( 'trx_addons_cpt_list',					'quickloans_trx_addons_cpt_list');
			add_filter( 'trx_addons_sc_list',					'quickloans_trx_addons_sc_list');
			add_filter( 'trx_addons_widgets_list',				'quickloans_trx_addons_widgets_list');
		}
	}
}

// CV
if ( !function_exists( 'quickloans_trx_addons_cv_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_cv_enable', 'quickloans_trx_addons_cv_enable');
	function quickloans_trx_addons_cv_enable($enable=false) {
...
// Demo mode
if ( !function_exists( 'quickloans_trx_addons_demo_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_demo_enable', 'quickloans_trx_addons_demo_enable');
	function quickloans_trx_addons_demo_enable($enable=false) {
		// To do: return false if theme not use CV functionality
		return true;
	}
}
...
// EDD Themes market
if ( !function_exists( 'quickloans_trx_addons_edd_themes_market_enable' ) ) {
	//Handler of the add_filter( 'trx_addons_filter_edd_themes_market', 'quickloans_trx_addons_edd_themes_market_enable')) {
	function quickloans_trx_addons_edd_themes_market_enable($enable=false) {
...
// CPT
if ( !function_exists( 'quickloans_trx_addons_cpt_list' ) ) {
	//Handler of the add_filter('trx_addons_cpt_list',	'quickloans_trx_addons_cpt_list');
	function quickloans_trx_addons_cpt_list($list=array()) {
...
// Shortcodes
if ( !function_exists( 'quickloans_trx_addons_sc_list' ) ) {
	//Handler of the add_filter('trx_addons_sc_list',	'quickloans_trx_addons_sc_list');
	function quickloans_trx_addons_sc_list($list=array()) {
...
// Widgets
if ( !function_exists( 'quickloans_trx_addons_widgets_list' ) ) {
	//Handler of the add_filter('trx_addons_widgets_list',	'quickloans_trx_addons_widgets_list');
	function quickloans_trx_addons_widgets_list($list=array()) {
...
if ( !function_exists( 'quickloans_trx_addons_menu_cache' ) ) {
	add_filter( 'trx_addons_filter_menu_cache', 'quickloans_trx_addons_menu_cache');
	function quickloans_trx_addons_menu_cache($list=array()) {
...
if (!function_exists('quickloans_trx_addons_localize_script')) {
	...
	function quickloans_trx_addons_localize_script($arr) {
		$arr['alter_link_color'] = quickloans_get_scheme_color('alter_link');
		return $arr;
	}
}
...
if ( !function_exists( 'quickloans_trx_addons_sc_type' ) ) {
	add_filter( 'trx_addons_sc_type', 'quickloans_trx_addons_sc_type', 10, 2);
	function quickloans_trx_addons_sc_type($list, $sc) {
        if ($sc == 'trx_sc_title') {
            $list[esc_html__('Modern', 'quickloans')] = 'modern';
        }
        if ($sc == 'trx_sc_button') {
            $list[esc_html__('Wide', 'quickloans')] = 'wide';
        }
        if ($sc == 'trx_sc_services') {
            $list[esc_html__('Wide', 'quickloans')] = 'wide';
            $list[esc_html__('Short', 'quickloans')] = 'short';
        }
        return $list;
	}
}
...
if ( !function_exists( 'quickloans_trx_addons_sc_atts' ) ) {
	add_filter( 'trx_addons_sc_atts', 'quickloans_trx_addons_sc_atts', 10, 2);
	function quickloans_trx_addons_sc_atts($atts, $sc) {
...
if ( !function_exists( 'quickloans_trx_addons_sc_map' ) ) {
	add_filter( 'trx_addons_sc_map', 'quickloans_trx_addons_sc_map', 10, 2);
	function quickloans_trx_addons_sc_map($params, $sc) {
		...
		if (in_array($sc, array('trx_sc_action', 'trx_sc_blogger', 'trx_sc_cars', 'trx_sc_courses', 'trx_sc_content', 'trx_sc_dishes',
			...
			$params['params'][] = array(
					'param_name' => 'scheme',
					'heading' => esc_html__('Color scheme', 'quickloans'),
					'description' => wp_kses_data( __('Select color scheme to decorate this block', 'quickloans') ),
					'group' => esc_html__('Colors', 'quickloans'),
					'admin_label' => true,
					'value' => array_flip(quickloans_get_list_schemes(true)),
					'type' => 'dropdown'
				);
		}
		// Param 'color_style'
		$param = array(
			...
			'heading' => esc_html__('Color style', 'quickloans'),
			'description' => wp_kses_data( __('Select color style to decorate this block', 'quickloans') ),
			'edit_field_class' => 'vc_col-sm-4',
			'admin_label' => true,
			'value' => array(
				esc_html__('Default', 'quickloans') => 'default',
				esc_html__('Link 2', 'quickloans') => 'link2',
				esc_html__('Link 3', 'quickloans') => 'link3',
				esc_html__('Dark', 'quickloans') => 'dark'
			),
			'type' => 'dropdown'
		);
        ...
        if ($sc=='trx_sc_layouts_iconed_text') {
            $params['params'][] = array(
                ...
                "heading" => esc_html__("Hide on notebook", 'quickloans'),
                "description" => wp_kses_data( __("Hide this block on the notebook", 'quickloans') ),
                "admin_label" => true,
                "std" => 0,
                "value" => array(
                    esc_html__("Hide on notebook", 'quickloans') => 1
                ),
                "type" => "checkbox"
            );
        }
...
if ( !function_exists( 'quickloans_trx_addons_sc_output' ) ) {
	add_filter( 'trx_addons_sc_output', 'quickloans_trx_addons_sc_output', 10, 4);
	function quickloans_trx_addons_sc_output($output, $sc, $atts, $content) {
		
		if (in_array($sc, array('trx_sc_action'))) {
			if (!empty($atts['scheme']) && !quickloans_is_inherit($atts['scheme']))
				$output = str_replace('class="sc_action ', 'class="sc_action scheme_'.esc_attr($atts['scheme']).' ', $output);
			if (!empty($atts['color_style']) && !quickloans_is_inherit($atts['color_style']))
        ...
        if ($sc=='trx_sc_layouts_iconed_text') {
            if (!empty($atts['hide_on_notebook'])) {
...
if ( !function_exists( 'quickloans_trx_addons_sc_item_title_tag' ) ) {
	add_filter( 'trx_addons_filter_sc_item_title_tag', 'quickloans_trx_addons_sc_item_title_tag');
	function quickloans_trx_addons_sc_item_title_tag($tag='') {
		return $tag=='h1' ? 'h2' : $tag;
	}
}

// Return args for the item's button
if ( !function_exists( 'quickloans_trx_addons_sc_item_button_args' ) ) {
	add_filter( 'trx_addons_filter_sc_item_button_args', 'quickloans_trx_addons_sc_item_button_args', 10, 3);
	function quickloans_trx_addons_sc_item_button_args($args, $sc, $sc_args) {
...
if ( !function_exists( 'quickloans_trx_addons_slider_title' ) ) {
	add_filter( 'trx_addons_filter_slider_title',	'quickloans_trx_addons_slider_title', 10, 2 );
	function quickloans_trx_addons_slider_title($title, $data) {
		$title = '';
		if (!empty($data['title'])) 
...
if ( !function_exists( 'quickloans_trx_addons_sc_googlemap_styles' ) ) {
	add_filter( 'trx_addons_filter_sc_googlemap_styles',	'quickloans_trx_addons_sc_googlemap_styles');
	function quickloans_trx_addons_sc_googlemap_styles($list) {
		$list[esc_html__('Dark', 'quickloans')] = 'dark';
		return $list;
	}
}
...
if ( !function_exists( 'quickloans_trx_addons_editor_init' ) ) {
	if (is_admin()) add_filter( 'tiny_mce_before_init', 'quickloans_trx_addons_editor_init', 11);
	function quickloans_trx_addons_editor_init($opt) {
		if (quickloans_exists_trx_addons()) {
            ...
            if (true && !empty($opt['style_formats'])) {
                ...
                if (is_array($style_formats) && count($style_formats)>0 ) {
                    foreach ($style_formats as $k=>$v) {
                        if ( $v['title'] == esc_html__('Inline', 'quickloans') ) {
                            $style_formats[$k]['items'][] = array(
                                'title' => esc_html__('Accent2 text', 'quickloans'),
                                'inline' => 'span',
                                'classes' => 'trx_addons_accent2'
                            );
                            $style_formats[$k]['items'][] = array(
                                'title' => esc_html__('Banner large', 'quickloans'),
                                'inline' => 'span',
                                'classes' => 'trx_addons_banner_large'
                            );
                            $style_formats[$k]['items'][] = array(
                                'title' => esc_html__('Banner medium', 'quickloans'),
                                'inline' => 'span',
                                'classes' => 'trx_addons_banner_medium'
                            );
                            $style_formats[$k]['items'][] = array(
                                'title' => esc_html__('Banner small', 'quickloans'),
                                'inline' => 'span',
                                'classes' => 'trx_addons_banner_small'
                            );
                        }
                    }
...
if ( !function_exists( 'quickloans_trx_addons_allow_override_header_image' ) ) {
	add_filter( 'quickloans_filter_allow_override_header_image', 'quickloans_trx_addons_allow_override_header_image' );
	function quickloans_trx_addons_allow_override_header_image($allow) {
		return quickloans_is_team_page() || quickloans_is_portfolio_page() ? false : $allow;
	}
}

// Hide sidebar on the team and portfolio pages
if ( !function_exists( 'quickloans_trx_addons_sidebar_present' ) ) {
	add_filter( 'quickloans_filter_sidebar_present', 'quickloans_trx_addons_sidebar_present' );
	function quickloans_trx_addons_sidebar_present($present) {
		return !is_single() && (quickloans_is_team_page() || quickloans_is_portfolio_page()) ? false : $present;
	}
}

// Get thumb size for the team items
if ( !function_exists( 'quickloans_trx_addons_thumb_size' ) ) {
	add_filter( 'trx_addons_filter_thumb_size',	'quickloans_trx_addons_thumb_size', 10, 2);
	function quickloans_trx_addons_thumb_size($thumb_size='', $type='') {
		if ($type == 'team-default')
			$thumb_size = quickloans_get_thumb_size('team-2');
		if ($type == 'team-featured')
			$thumb_size = quickloans_get_thumb_size('team');
        if ($type == 'services-short')
            $thumb_size = quickloans_get_thumb_size('full');
        if ($type == 'services-default')
            $thumb_size = quickloans_get_thumb_size('blogger');
		return $thumb_size;
	}
}
...
if (!function_exists('quickloans_trx_addons_meta_box_fields')) {
	add_filter( 'trx_addons_filter_meta_box_fields', 'quickloans_trx_addons_meta_box_fields', 10, 2);
	function quickloans_trx_addons_meta_box_fields($mb, $post_type) {
		if (defined('TRX_ADDONS_CPT_TEAM_PT') && $post_type==TRX_ADDONS_CPT_TEAM_PT) {
			$mb['email'] = array(
				"title" => esc_html__("E-mail",  'quickloans'),
				"desc" => wp_kses_data( __("Team member's email", 'quickloans') ),
				"std" => "",
				"details" => true,
				"type" => "text"
			);
