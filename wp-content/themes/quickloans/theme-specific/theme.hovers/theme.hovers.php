...
if (!function_exists('quickloans_hovers_theme_setup3')) {
	add_action( 'after_setup_theme', 'quickloans_hovers_theme_setup3', 3 );
	function quickloans_hovers_theme_setup3() {

		// Add 'Buttons hover' option
		quickloans_storage_set_array_after('options', 'border_radius', array(
			'button_hover' => array(
				"title" => esc_html__("Button's hover", 'quickloans'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme buttons', 'quickloans') ),
				"std" => 'slide_top',
				"options" => array(
					'default'		=> esc_html__('Fade',				'quickloans'),
					'slide_left'	=> esc_html__('Slide from Left',	'quickloans'),
					'slide_right'	=> esc_html__('Slide from Right',	'quickloans'),
					'slide_top'		=> esc_html__('Slide from Top',		'quickloans'),
					'slide_bottom'	=> esc_html__('Slide from Bottom',	'quickloans'),
//					'arrow'			=> esc_html__('Arrow',				'quickloans'),
				),
				"type" => "select"
			),
			'image_hover' => array(
				"title" => esc_html__("Image's hover", 'quickloans'),
				"desc" => wp_kses_data( __('Select hover effect to decorate all theme images', 'quickloans') ),
				"std" => 'dots',
				"override" => array(
					'mode' => 'page',
				...
				"options" => array(
					'dots'	=> esc_html__('Dots',	'quickloans'),
					'icon'	=> esc_html__('Icon',	'quickloans'),
					'icons'	=> esc_html__('Icons',	'quickloans'),
					'zoom'	=> esc_html__('Zoom',	'quickloans'),
					'fade'	=> esc_html__('Fade',	'quickloans'),
					'slide'	=> esc_html__('Slide',	'quickloans'),
					'pull'	=> esc_html__('Pull',	'quickloans'),
					'border'=> esc_html__('Border',	'quickloans')
				),
				"type" => "select"
			) )
		);
...
if (!function_exists('quickloans_hovers_theme_setup9')) {
	add_action( 'after_setup_theme', 'quickloans_hovers_theme_setup9', 9 );
	function quickloans_hovers_theme_setup9() {
		add_action( 'wp_enqueue_scripts',		'quickloans_hovers_frontend_scripts', 1010 );
		add_filter( 'quickloans_filter_localize_script','quickloans_hovers_localize_script' );
		add_filter( 'quickloans_filter_merge_scripts',	'quickloans_hovers_merge_scripts' );
		add_filter( 'quickloans_filter_merge_styles',	'quickloans_hovers_merge_styles' );
		add_filter( 'quickloans_filter_get_css', 		'quickloans_hovers_get_css', 10, 4 );
	}
}
	
// Enqueue hover styles and scripts
if ( !function_exists( 'quickloans_hovers_frontend_scripts' ) ) {
	//Handler of the add_action( 'wp_enqueue_scripts', 'quickloans_hovers_frontend_scripts', 1010 );
	function quickloans_hovers_frontend_scripts() {
		if ( quickloans_is_on(quickloans_get_theme_option('debug_mode')) && quickloans_get_file_dir('theme-specific/theme.hovers/theme.hovers.js')!='' )
			wp_enqueue_script( 'quickloans-hovers', quickloans_get_file_url('theme-specific/theme.hovers/theme.hovers.js'), array('jquery'), null, true );
		if ( quickloans_is_on(quickloans_get_theme_option('debug_mode')) && quickloans_get_file_dir('theme-specific/theme.hovers/theme.hovers.css')!='' )
			wp_enqueue_style( 'quickloans-hovers',  quickloans_get_file_url('theme-specific/theme.hovers/theme.hovers.css'), array(), null );
	}
}

// Merge hover effects into single js
if (!function_exists('quickloans_hovers_merge_scripts')) {
	//Handler of the add_filter( 'quickloans_filter_merge_scripts', 'quickloans_hovers_merge_scripts' );
	function quickloans_hovers_merge_scripts($list) {
...
if (!function_exists('quickloans_hovers_merge_styles')) {
	//Handler of the add_filter( 'quickloans_filter_merge_styles', 'quickloans_hovers_merge_styles' );
	function quickloans_hovers_merge_styles($list) {
...
if (!function_exists('quickloans_hovers_localize_script')) {
	//Handler of the add_filter( 'quickloans_filter_localize_script','quickloans_hovers_localize_script' );
	function quickloans_hovers_localize_script($arr) {
		$arr['button_hover'] = quickloans_get_theme_option('button_hover');
		return $arr;
	}
}

// Add hover icons on the featured image
if ( !function_exists('quickloans_hovers_add_icons') ) {
	function quickloans_hovers_add_icons($hover, $args=array()) {
					...
					echo apply_filters( 'woocommerce_loop_add_to_cart_link',
										'<a rel="nofollow" href="' . esc_url($product->add_to_cart_url()) . '" 
														...
														class="shop_cart icon-cart-2 button add_to_cart_button'
																...
																. '">'
											. ($hover == 'shop_buttons' ? esc_html__('Buy now', 'quickloans') : '')
										. '</a>',
										$product );
				}
				?><a href="<?php echo esc_url(is_object($args['cat']) ? get_term_link($args['cat']->slug, 'product_cat') : get_permalink()); ?>" aria-hidden="true" class="shop_link button icon-link"><?php
					if ($hover == 'shop_buttons') {
						if (is_object($args['cat']))
							esc_html_e('View products', 'quickloans');
						else
							esc_html_e('Details', 'quickloans');
					}
				?></a>
			</div>
			<?php
...
// Add styles into CSS
if ( !function_exists( 'quickloans_hovers_get_css' ) ) {
	//Handler of the add_filter( 'quickloans_filter_get_css', 'quickloans_hovers_get_css', 10, 4 );
	function quickloans_hovers_get_css($css, $colors, $fonts, $scheme='') {
...
.sc_button_hover_slide_top:hover:not([disabled]),
.sc_button_hover_slide_top.active,
.ui-state-active .sc_button_hover_slide_top,
.vc_active .sc_button_hover_slide_top:hover,
