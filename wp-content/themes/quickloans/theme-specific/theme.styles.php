...
if (!function_exists('quickloans_customizer_get_css')) {

	function quickloans_customizer_get_css($colors=null, $fonts=null, $remove_spaces=true, $only_scheme='') {
		...
		if ($fonts) {
			...
			$fonts = quickloans_customizer_add_theme_fonts($fonts);
...
EOT;
			$rez = apply_filters('quickloans_filter_get_css', $rez, false, $fonts, '');
			$css['fonts'] = $rez['fonts'];
...
EOT;
		...
		//--------------------------------------
		if ($colors !== false) {
			$schemes = empty($only_scheme) ? array_keys(quickloans_get_list_schemes()) : array($only_scheme);
	
			if (count($schemes) > 0) {
				$rez = array();
				foreach ($schemes as $scheme) {
					// Prepare colors
					if (empty($only_scheme)) $colors = quickloans_get_scheme_colors($scheme);
...
blockquote {
	...
	background-image: linear-gradient(-270deg, {$colors['text_link']} 0%, {$colors['text_hover']} 100%);
...
/* Normal button */
button:not(.esgbox-toolbar .esgbox-button),
input[type="reset"],
input[type="submit"],
input[type="button"],
.post_item .more-link,
...
.quickloans_tabs .quickloans_tabs_titles li a:hover {
	color: {$colors['inverse_link']};
	background-color: {$colors['text_link']};
}
div.esg-filter-wrapper .esg-filterbutton.selected > span,
.quickloans_tabs .quickloans_tabs_titles li.ui-state-active a {
	color: {$colors['bg_color']};
	background-color: {$colors['text_dark']};
}
...
/* Single post */
.post_item_single a.post_meta_item.post_author {
    color: {$colors['text_link2']};
}
.post_item_single a.post_meta_item.post_author:hover {
    color: {$colors['text_link']};
}
...
.scheme_self.sidebar .widget.widget_search .widget_title {
    color: {$colors['bg_color']};
    background-color: {$colors['bg_color_0']};
}
.scheme_self.sidebar .widget.widget_search {
    color: {$colors['bg_color']};
    background-color: {$colors['text_link']};
...
.sidebar .widget.widget_search input[type="search"]::-webkit-input-placeholder { color: {$colors['bg_color']}; }
.sidebar .widget.widget_search input[type="search"]::-moz-placeholder          { color: {$colors['bg_color']}; }
.sidebar .widget.widget_search input[type="search"]:-ms-input-placeholder      { color: {$colors['bg_color']}; }
...
EOT;
					...
					$rez = apply_filters('quickloans_filter_get_css', $rez, $colors, false, $scheme);
		...
		return apply_filters( 'quickloans_filter_prepare_css', $css_str, $remove_spaces );
	}
}
?>